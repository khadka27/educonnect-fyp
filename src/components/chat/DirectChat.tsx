...
const ChatInterface = () => {
  ...
  const handleSendMessage = useCallback(() => {
    if (newMessage.trim() && selectedUser && session?.user?.id) {
      const newMsgId = _.uniqueId("msg_"); // Use lodash to generate a temporary unique ID
      const msg = {
        id: newMsgId,
        content: newMessage,
        senderId: session.user.id,
        receiverId: selectedUser.id,
        createdAt: new Date().toISOString(), // Temporary timestamp for optimistic rendering
      };
